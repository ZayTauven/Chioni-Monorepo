...
- **Backend complet jusqu'à la couche API/services** : venv `backend/.venv` (Python 3.13), settings via `backend/.env` (django-environ), `/api/v1/health/` public, Swagger sur `/api/docs/`, DRF deny-by-default.
  ...
  - **445 tests verts** (`pytest` depuis `backend/`, ~40 s), dont 3 campagnes adversariales conservées en régression (`test_hardening.py`, `test_adversarial_api_recheck.py`). ADR 0001→0011 dans `docs/adr/`.
  ...
  - **Outil dev** : `python manage.py simulate_psp_payment [--latest] [--fail]` rejoue le webhook PSP signé en dev (DEBUG + PSP fake obligatoires) — rend le flux de paiement démontrable de bout en bout.
