...
- Reste à faire (dans l'ordre) : auth téléphone+OTP SMS (réutiliser le throttling en place, plafonds stricts — coût SMS), adaptation du frontend aux 3 espaces (élagage de Vireo + branchement API), notifications SMS/e-mail (Celery), intégration Stripe réelle (`psp/stripe.py`, chantier dédié avec clés + `Stripe-Signature`), maquettes des parcours, étude paiement partenaire local (risque n° 1), étude du module IA de reprise du papier. Vigilances actées par le guardian : ne jamais exposer `Dispute.reason` à l'autre partie si des vues litige patient/tuteur apparaissent ; règle « pas de PII dans les payloads d'audit » = contrat de revue permanent.
