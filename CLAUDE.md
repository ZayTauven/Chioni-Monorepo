...
- Reste à faire (dans l'ordre) : **chantier « Gâter les centres » vagues 2–3** (2a caisse guichet : encaissement espèces/mobile money → ledger — nécessite un cadrage : nouveaux comptes de ledger + reçu guichet KMF distinct du `Receipt` diaspora EUR-centrique — ADR dédié + revue guardian ; 2b pilotage : stats réelles activité/recettes/impayés ; 3 frontend centre : agenda, file du jour, caisse, dashboard piloté, réglages logo/photo + revue UX care « efficacité guichet »), test manuel de bout en bout des 3 espaces contre le backend local (parcours démo : OTP → porte B → invitation tuteur → demande → devis → pay → `simulate_psp_payment` → reçu ; `seed_demo` fournit les comptes), intégration Stripe réelle (`psp/stripe.py` — exigences actées dans l'addendum ADR 0009 : annulation de l'intent périmé côté PSP, appel HTTP hors transaction) + backend SMS agrégateur comorien (chantiers dédiés avec clés ; y traiter le retry SMS borné + dead-letter et le lien d'information/opposition art. 14 — ADR 0012), ré-établissement d'un lien de tutelle révoqué (limitation de schéma assumée, l'UI le dit honnêtement), étude paiement partenaire local (risque n° 1), étude du module IA de reprise du papier. Vigilances actées par le guardian : ne jamais exposer `Dispute.reason` à l'autre partie si des vues litige patient/tuteur apparaissent ; règle « pas de PII dans les payloads d'audit » = contrat de revue permanent ; jamais de HTML dynamique côté frontend (tokens en localStorage) + CSP à poser au déploiement ; le contenu d'un SMS suit la visibilité dans l'app, jamais l'inverse. Vigilances vague 1 (guardian) : identité d'un compte ombre partagé entre tenants éditable par chaque directeur (à trancher si le cas apparaît) ; pas de throttle dédié sur les uploads (CPU Pillow — à couvrir avec un throttle user global) ; `/media/` en prod = front web server + `nosniff` (contrat ADR 0014) ; RDV annulé/déplacé après le rappel J-1 → pas de SMS correctif (limitation produit assumée, ADR 0013).
