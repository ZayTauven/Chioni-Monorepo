...
| `POST /auth/me/erasure-request/cancel/` | **corps vide** | 200 même shape (`status: "annulee"`) | **SV — rétractation (art. 12)** : la personne retire sa PROPRE demande tant qu'elle est `en_attente`. Aucune demande en attente → 400 `["Aucune demande d'effacement en attente pour votre compte."]`. Après annulation, redemander est possible (nouvelle ligne). |
...
- `status` : `en_attente` | `traitee` | `refusee` | `annulee` (**SV** — annulée par la personne elle-même via `cancel/`).
...
- `patient` (si profil revendiqué) : `{profile, guardian_links, appointments, encounters, prescriptions, record_entries, medical_file, vital_signs, documents, insurances, payment_requests, receipts, cash_receipts, stays, contact_preferences, availability}` — items **identiques** aux endpoints `/patients/me/*` correspondants. `documents` = **métadonnées seules**, jamais d'URL ni d'octets (contrat ADR 0016 §5 inchangé) ; documents archivés exclus, comme dans la liste patient. **SV** : `stays` = fenêtre patient de `/patients/me/stays/` (ni lit, ni priorité, ni motif d'annulation) ; `contact_preferences` = forme constante de `/patients/me/contact-preferences/` ; `availability` = `[{prescription: <id>, requests: [...]}]` au payload patient de `/patients/me/prescriptions/{pk}/availability/` (jamais le commentaire d'officine ni les compteurs de diffusion).
...
- `center_staff` : `{memberships: [...], hr: [...]}` au format de `/auth/me/` — **les memberships du salarié, pas les données de son centre**. **SV** : `hr` = le dossier RH DE LA PERSONNE, centre par centre (`[{employment, attendance, leave_requests, leave_documents}]`, fenêtres des routes `/centers/{c}/hrm/me/*`) — uniquement dans les centres où elle est ENCORE membre actif ; `leave_documents` = métadonnées seules ; `[]` tant qu'aucun dossier RH n'est ouvert.
...
- `GET /patients/me/prescriptions/` → `{id, encounter, status(emise|delivree), items[{id,medication,dosage}], delivered_at(ISO|null), created_at}` — **S9** : `delivered_at` se lit « vos médicaments vous ont été remis le … » ; **pas de `delivered_by`** (aucune identité de personnel ne traverse une vue patient).
- **`GET /patients/me/prescriptions/{pk}/availability/` (S9, ADR 0022)** — « où trouver mes médicaments ? », **tableau nu** (non paginé), ordonnance d'autrui → 404. Item : `{id, status(ouverte|close), city, island, items[{id,medication}], responses[{id, pharmacy{id,name,island,city,address,phone}, lines[{item,is_available}], created_at}], created_at, expires_at}`.
  - **Lecture seule : le patient ne lance pas de recherche** (arbitrage PO S9 — c'est le médecin ou le pharmacien du centre, en fin de consultation). Ne pas construire de bouton « chercher mes médicaments ».
  - **Pas de `comment`** : le commentaire libre d'une officine s'arrête au centre (ADR 0022 décision 3). Ne pas prévoir d'emplacement.
  - Vocabulaire d'écran : `is_available: true` → « disponible ici », `false` → « pas disponible ». Le téléphone de la pharmacie est **le geste utile** — un lien `tel:` avant tout le reste. Une recherche `close` (fermée par le centre ou périmée au bout de 48 h) : dire « cette recherche est terminée », jamais un rouge d'échec, et les réponses restent lisibles.
...
- **S2 — Consentement clinique recueilli au guichet (ADR 0004 addendum + addendum SV)** : `POST /centers/{c}/patients/{pk}/consents/clinical/` (rôles BILLING) `{"guardian_link": <id>, "collected_via": "papier"}` → 201 `{guardian_link, scope:"detail_clinique", collected_via, granted_at, revoked_at:null}` ; `DELETE` (retrait) `{"guardian_link"}` → 200 (même shape, `revoked_at` rempli). **SV.1.2 — papier signé OBLIGATOIRE** : `"oral"` n'est PLUS accepté → 400 `{"collected_via": ["Mode de recueil invalide : seul le formulaire papier signé est accepté au guichet."]}` — **retirer l'option `oral` de l'UI** (l'historique `oral` reste lisible en GET). **Patients NON revendiqués UNIQUEMENT** — patient revendiqué → 400 `["Ce patient gère lui-même ses consentements depuis son espace."]` (afficher tel quel : le patient passe par SON espace) ; patient hors périmètre → 404 ; `guardian_link` qui n'est pas un lien `actif` de CE patient (étranger, révoqué, en attente, inexistant) → 400 unique `["Ce lien de tutelle n'est pas un lien actif de ce patient."]` ; déjà accordé → 400 ; retrait sans consentement actif → 400. **SV.1.1 — lien fabriqué par le centre** : un lien né d'une invitation émise par CE centre (porte C) → 400 `["Ce lien de tutelle est né d'une invitation émise par le centre : le consentement clinique ne peut pas être recueilli au guichet du centre qui a lui-même initié le lien."]` — afficher tel quel, ce n'est pas une erreur de saisie ; un AUTRE centre reste libre ; le RETRAIT reste possible. Le `guardian_link` vient de `GET .../guardian-links/` ci-dessus. À la revendication du profil par le patient, ce consentement est automatiquement révoqué (porte de confirmation du titulaire) — ne pas le présenter comme permanent. **SV — état persistant** : `GET` (mêmes rôles) → tableau nu `[{guardian_link, scope, collected_via, granted_at, revoked_at:null}]` des consentements guichet ACTIFS de ce patient (les consentements accordés par le patient depuis son espace ne transitent jamais ici) ; patient revendiqué → 200 `[]` (pas un 400 : lire n'est pas octroyer) — l'UI peut enfin afficher l'état au chargement, plus seulement le résultat de sa session.
...
- **Caisse (ADR 0015 — rôles BILLING)** :
  ...
  - `POST .../payments/{pk}/reverse/` `{"reason"*}` → 201 (item avec `reversal` rempli : `{id, cash_payment, invoice(**SV** — l'id de facture : la contre-passation du journal devient enfin navigable), method, amount_kmf, reason, reversed_by, reversed_by_display(**SV** — même règle que `received_by_display`), ledger_transaction, created_at}`). **Jamais d'annulation** : contre-passation unique par encaissement (2e → 400), motif obligatoire, `pont_confiance` jamais contre-passable au guichet (→ litige). Effet honnête : facture `payee` → `emise` si le solde redevient > 0.
...
### Réseau des pharmacies vu du centre (S9, ADR 0022)

`GET /centers/{c}/pharmacies/?island=&city=&q=` (**tableau nu**, TOUT staff actif) · `GET|POST /centers/{c}/prescriptions/{p}/availability-requests/` (rôles cliniques + pharmacien) · `GET /centers/{c}/availability-requests/?status=&prescription=` (paginé) · `GET .../availability-requests/{pk}/` · `POST .../availability-requests/{pk}/close/`.

- **Annuaire** : `{id, name, island, city, address, phone}` — officines **validées** seulement. Cet écran est aussi celui qui **annonce la diffusion** : avant d'envoyer, montrer la liste réelle des pharmacies qui vont recevoir, pas un compteur abstrait.
- **POST** `{island*, city?, item_ids*[]}` → 201. `item_ids` est **obligatoire et explicite** : les cases sont cochées par défaut à l'écran, mais décocher est le geste que l'ADR protège — **le prescripteur choisit ce qui sort**. Prévoir une phrase courte à côté : « ces médicaments seront envoyés aux pharmacies de la zone, sans le nom du patient ».
- Refus, tous en 400 avec un message actionnable à afficher tel quel : ordonnance déjà délivrée · aucune ligne cochée · ligne étrangère à l'ordonnance · **aucune pharmacie validée dans cette zone** · **diffusion au-delà du plafond** (« N pharmacies correspondent…, précisez la commune ») · **recherche déjà ouverte** sur la même ordonnance et la même zone (le double-clic ne diffuse pas deux fois).
- Item demande : `{id, prescription, island, city, status(ouverte|close), close_reason(manuelle|peremption|""), items[{id,medication}], recipient_count, response_count, last_response_at, created_at, expires_at, closed_at}` ; le DÉTAIL ajoute `responses[{id, pharmacy{…}, comment, lines[{item,is_available}], created_at}]`.
  - `recipient_count`/`response_count`/`last_response_at` valent `0`/`null` dans la réponse du POST (l'objet n'y est pas annoté) — refetcher la liste après création.
  - **Plusieurs réponses d'une même officine sont normales** (le stock bouge) : la plus récente fait foi, l'historique reste. Afficher la dernière, et ne pas présenter une correction comme une contradiction.
  - `expires_at` : 48 h. Une recherche périmée se ferme toute seule (`close_reason: "peremption"`) — le dire simplement (« sans réponse au bout de 48 h »), et proposer d'en relancer une.
- **Ce que la pharmacie NE voit PAS, et qu'il faut dire à l'écran** : ni le nom du centre, ni le patient, ni la posologie. C'est l'argument qui fera utiliser la fonction — l'écrire, pas le sous-entendre.

### Le comptoir du pharmacien (S9 — dette C.1 de l'audit, soldée)

`GET /centers/{c}/prescriptions/?patient=&status=&date=` (paginé, rôles cliniques + pharmacien) · `POST /centers/{c}/prescriptions/{pk}/deliver/`.

- Item : `{id, encounter, status(emise|delivree), items[{id,medication,dosage}], delivered_at, created_at}`.
...
- **File « à relancer »** : `{id, patient, patient_name, patient_phone_masked, total_kmf, paid_kmf, balance_kmf, age_days, created_at, reminders_sent, reminders_exhausted, last_contact_at, last_human_contact_at(**SV** — le QUAND du dernier contact HUMAIN, celui qui porte une issue ; `last_contact_at` reste le dernier contact de toute nature, SMS automatique compris ; nullable), last_outcome, guardian_reachable}`. Tri `?ordering=` ∈ `-age|age|-balance|balance` (défaut `-age`), valeur inconnue → 400 par champ.
...
- **`POST .../crm/contacts/`** `{kind*(relance_impaye|reprise_contact), channel*(appel|guichet), recipient*(patient|tuteur), outcome*(joint|sans_reponse|promesse_de_reglement|a_rappeler|**rdv_fixe** — SV : « Rendez-vous fixé », l'issue heureuse d'une reprise de contact), invoice?, appointment?}` → 201 `{id, patient, kind, channel, recipient, invoice, appointment, outcome, created_by, created_at}`.
...
- Item : `{id, number("E-000004"), sequence_number, period_start, period_end, total_collected_kmf, total_reversed_kmf, net_kmf, line_count, generated_by, created_at}` ; le DÉTAIL ajoute `lines[]` **et `generated_by_display`** (**SV** — « un humain signe la photo » : le nom complet de qui a généré la pièce, sur le détail SEUL, la liste garde l'id nu).
...
| Endpoint | Rôle | Notes |
|---|---|---|
| `GET /platform/centers/?kyc_status=&q=` | support+admin | Liste paginée de TOUS les centres. `q` : nom ou ville. `kyc_status` inconnu → 400 `{"kyc_status": […]}`. **SV** : l'item expose `logo` (url\|null) — le papier de facture plateforme peut enfin rendre le vrai logo. Limite : les réponses `POST /platform/centers/` et `POST .../kyc/` rendent une URL **relative** (pas de contexte request) — la liste et le détail rendent l'URL absolue. |
...
### Réseau des pharmacies (`/platform/pharmacies/`, S9, ADR 0022)

`GET|POST /platform/pharmacies/?status=&island=&q=` · `GET /platform/pharmacies/similar/?name=&city=&island=` · `GET /platform/pharmacies/{pk}/` · `POST .../{pk}/status/` · `POST .../{pk}/members/` · `GET .../{pk}/documents/` (+ `/{id}/download/`).

Item : `{id, name, island, city, address, phone, email, status, status_reason, status_updated_at, member_active_count, document_count, received_request_count, created_at}` — **exactement ces champs**.

- **`admin` seul écrit** (enregistrer, décider un statut, amorcer un membre) ; `support` lit. Même partage que les centres.
- **La validation est le seul vrai pouvoir de ce module** : elle ouvre la réception des demandes. L'écran doit montrer les **pièces** (`documents/`) avant le bouton — valider une officine sans licence est du théâtre.
- Machine fermée : `en_attente → validee|suspendue`, `validee → suspendue`, `suspendue → validee`. **Motif obligatoire pour suspendre** (400 sinon) ; il est lu par l'officine elle-même — l'écrire comme une consigne actionnable, jamais comme une note interne. Il n'entre dans aucun journal d'audit.
- **Ce qu'une suspension ferme, et rien de plus** : recevoir de nouvelles demandes, et y répondre. L'officine garde la lecture de son historique et le dépôt de ses pièces — le dire dans la modale, comme pour un centre.
- **Aucun médicament n'est visible côté plateforme** : superviser des officines n'est pas lire les ordonnances du pays. `received_request_count` est un compteur, il n'ouvre sur rien.
- Le premier membre naît en **compte ombre** claimé par OTP. Séparation des pouvoirs : un exploitant Chioni **ne peut pas** être inscrit dans une officine (400 explicite) — celui qui valide ne valide pas la sienne.
...
## Espace pharmacie — le 5ᵉ espace (`/pharmacy/{pharmacy_pk}/…`, S9 ADR 0022)

**Porte unique : `pharmacy_memberships` non vide dans `/auth/me/`** (`[{id, pharmacy{id, name, island, city, status}}]` — une LISTE : une même personne peut tenir deux officines). Anonyme → 401 ; authentifié sans appartenance → **403** `"Réservé aux pharmacies du réseau Chioni."` ; officine d'un autre → **404**.

> **Invariant de sprint, à tenir dans le code du frontend comme dans les types** : une officine connaît une **zone**, une **liste de médicaments** et un **numéro de demande**. Jamais le centre, jamais l'ordonnance, jamais le patient. Le backend ne les envoie pas — ne pas inventer d'emplacement, ne pas « enrichir » un écran avec un nom de clinique.

- `GET|PATCH /pharmacy/{p}/` — sa fiche : `{id, name, island, city, address, phone, email, status(en_attente|validee|suspendue), status_reason, status_updated_at, created_at}`. PATCH : `name, island, city, address, phone, email` — **jamais `status`** (envoyé quand même, il est ignoré). `status_reason` est la consigne écrite par Chioni pour une suspension : l'afficher en toutes lettres, c'est ce qui permet de se régulariser. **Attention (revue guardian S9) : modifier `island` ou `city` sur une officine `validee` la renvoie à `en_attente`** — la commune n'est pas une ligne d'adresse, c'est ce qui borne la circulation d'une liste de médicaments, donc elle se revérifie. Le PATCH répond 200 avec le nouveau statut et le `status_reason` qui l'explique : **prévenir AVANT** (« déclarer un déménagement suspend la réception des demandes jusqu'à vérification par Chioni ») plutôt que de laisser l'officine découvrir sa boîte vide. Corriger l'adresse, le téléphone, l'e-mail ou le nom ne change rien au statut.
- **L'écran d'accueil dit la vérité sur le statut** : `en_attente` → « votre inscription est en cours d'examen, vous ne recevez pas encore de demandes » (surtout pas une boîte vide qui ressemble à une panne) ; `suspendue` → dire ce qui continue (lire son historique, déposer ses pièces) avant ce qui est fermé (recevoir et répondre).
- `GET /pharmacy/{p}/requests/?status=` (paginé) · `GET .../requests/{pk}/` — item : `{id, island, city, status, created_at, expires_at, items[{id,medication}], my_response{id, comment, lines[{item,is_available}], created_at}|null}`. `id` est celui de la **ligne de diffusion**, pas de la demande.
- `POST .../requests/{pk}/respond/` `{lines[{item,is_available}]*, comment?}` → 200 (l'item à jour). **Toutes les lignes sont obligatoires** (400 « Répondez pour chacun des médicaments demandés. ») : un formulaire à deux boutons par ligne, pas de case à cocher qui laisserait un doute entre « non répondu » et « pas disponible ».
- **Répondre à nouveau est prévu et normal** : le stock bouge. Le bouton reste « Mettre à jour ma réponse », jamais grisé après le premier envoi.
- Refus à afficher tels quels : demande **close** · demande **expirée** (48 h) · officine **non validée** (« votre officine ne peut pas répondre pour le moment » — enchaîner sur `status_reason`).
- `GET|POST /pharmacy/{p}/members/` (**tableau nu**) · `POST .../members/{pk}/deactivate/` — `{id, display_name, is_active, created_at}`. **Aucun rôle, aucune hiérarchie** : tout membre actif inscrit un collègue (compte ombre + OTP, aucun mot de passe transmis) et peut se retirer, sauf s'il est **le dernier actif** (400, message actionnable).
- `GET|POST /pharmacy/{p}/documents/` (**tableau nu**) · `GET .../documents/{pk}/download/` · `POST .../documents/{pk}/archive/` — `{id, doc_type(registre_commerce|licence_officine|piece_identite_responsable|autre), created_at, archived_at}`. **Jamais d'URL de fichier** (stockage privé) : `apiDownload` comme les documents patients. POST **multipart**, throttle `uploads`, JPEG/PNG/WebP réels ≤ 2 Mo — **le PDF est refusé**. L'archivage est **définitif**.
- **Chrome attendu** : sobre, dérivé de l'espace plateforme (pas de ⌘K, pas de customizer, `CenterContext` jamais monté), **mobile-first** — une officine comorienne répond depuis un téléphone, au comptoir.

## N'existe PAS côté API (ne pas construire d'écran branché dessus)
Prise de RDV par le patient (self-booking — la lecture et l'annulation d'un `prevu` existent depuis S2, voir Espace patient ; la prise reste au guichet), agenda côté tuteur (aucun accès tuteur aux RDV), lecture du ledger, **miroir centre/tuteur d'un paiement diaspora refusé** (hors périmètre S4 — le directeur voit la ligne brute dans son journal d'audit, ce n'est pas un écran d'explication), **annulation d'une demande d'effacement par la personne** (S4 lot 3 : on dépose, on ne retire pas — à rouvrir si le terrain le demande), **restauration d'un compte anonymisé** (structurellement impossible : il n'y a plus rien à restaurer), **inscription en self-service d'un centre** (un centre entre toujours par l'exploitant), **paiement de sa facture d'abonnement par le centre depuis l'app** (le règlement se fait hors ligne, l'exploitant l'enregistre — les rails comoriens ne sont pas instruits), **export comptable / PDF des factures d'abonnement** (hors périmètre S5), **paiement en ligne de l'abonnement par le centre** (rails comoriens non instruits — hors périmètre acté), **édition d'un centre par la plateforme** (le profil du centre reste au directeur ; la plateforme décide le KYC, pas l'adresse), **annuaire du personnel côté plateforme** (compteurs seulement : la plateforme gouverne le tenant, elle ne consulte pas ses salariés), **support ouvert aux patients et aux tuteurs** (hors périmètre S5 acté — la tab bar lite est déjà pleine à 4 onglets), **notification d'une réponse de support** (ni SMS ni e-mail : le centre revoit son écran), **assignation / SLA / recherche plein texte** dans la file de support, **archivage d'une pièce jointe de ticket** (pas de chemin de correction, contrairement aux pièces KYC et aux documents patients), **suppression d'un exploitant** (on révoque, on ne supprime pas), reset de mot de passe, messagerie, **toute donnée S3 côté tuteur** (fiche médicale, signes vitaux, documents, assurances, identité élargie — verrou de sprint ADR 0016 : rien de nouveau n'est exposé au tuteur, même porteur de `detail_clinique` ; le payload protégé reste `{id, first_name, last_name, claim_status}`), upload PDF (différé — photos JPEG/PNG/WebP seulement), directives anticipées (hors périmètre S3, cadrage dédié à venir), **toute forme de PAIE** (barèmes, salaires, bulletins, taux — hors périmètre S7 acté, cadrage séparé : aucun champ de rémunération n'existe côté serveur et il ne faut en afficher aucun), **délégué RH** (le directeur assume ou délègue hors application — ne pas prévoir de rôle « responsable RH »), **pointage horaire** (arrivée/départ, badgeuse, géolocalisation : la feuille note une JOURNÉE, jamais des minutes), **soldes de congés calculés et reports** d'une année sur l'autre (`days` est un affichage, pas un solde), **plannings de garde prévisionnels** (roster), **contrats et documents d'embauche**, **évaluations**, **pré-remplissage des fériés nationaux** (le calendrier appartient au centre), **annulation d'un congé déjà approuvé** (S7 : les trois issues sont terminales — la feuille de présence fait foi). **S9 — réseau des pharmacies** : **prix des médicaments** (aucun champ côté serveur : un prix affiché à un patient devient une promesse, et un classement ferait de Chioni un comparateur — arbitrage réversible, à rouvrir avec son propre cadrage), **stock déclaré / catalogue consultable** (arbitrage PO : un catalogue non tenu à jour ment, et son mensonge coûte un déplacement), **recherche lancée par le patient** (le centre lance, le patient consulte), **notification du patient quand un médicament est disponible** (le prolongement le plus désirable, et le plus délicat à écrire sans rien dire du traitement — passage dédié), **demande libre hors ordonnance** (aucun champ de texte ne part vers un tiers, par conception), **délivrance partielle ligne par ligne**, **substitution générique**, **stock d'une pharmacie interne au centre**, **inscription en self-service d'une officine** (elle entre par l'exploitant, comme un centre), **pharmacie de garde**, **carte / géolocalisation / distance** (le ciblage est administratif : île + commune, jamais des coordonnées), **annuaire des pharmacies côté tuteur et côté patient** (le patient lit les réponses à SES recherches, il ne parcourt pas le réseau), et **toute route pharmacie côté tuteur** (verrou S3 intact : la disponibilité d'un traitement est une information clinique). La photo de profil n'apparaît JAMAIS dans les vues croisées patient/tuteur (le tuteur ne voit pas la photo du patient ni l'inverse) : ne pas prévoir d'emplacement pour. Les écrans correspondants sont soit à exclure du MVP frontend, soit des placeholders « bientôt » clairement assumés.
