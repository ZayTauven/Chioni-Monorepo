...
- `GET .../{pk}/quote/` (**GET**) → devis FX :
  ...
  **Le devis porte le SOLDE RESTANT de la facture** (ADR 0015 — des tranches guichet ont pu la régler en partie) : `amount_kmf` peut être < `total_kmf` de la demande — c'est le devis qui fait foi pour « ce que je paie », jamais `total_kmf`. Facture déjà réglée au guichet → 400 `"Cette facture est déjà entièrement réglée : plus rien à payer."` (la demande peut rester `envoyee` — afficher ce 400 honnêtement).
