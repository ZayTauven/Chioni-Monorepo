...
- **S7 (guardian, 15/08/2026) — ARBITRAGE À CONFIRMER** : le planning collectif est désormais borné à **±31 jours** (`SCHEDULE_WINDOW_DAYS`). Sans borne, n'importe quel membre du personnel pouvait interroger jour par jour toute l'année écoulée et reconstituer, nom par nom, le **relevé d'absentéisme de tout le service** — le champ ne disait rien, mais l'agrégat était l'instrument que l'arbitrage PO n° 2 refuse d'inventer. Chacun garde son propre historique sur 366 j, le directeur garde sa feuille. **Réversible** si le PO veut une fenêtre plus large.
...
- **S7 (guardian)** : `request_leave` sans horizon (un congé en 2999 est accepté) ; `update_employment` ne revalide pas les présences/congés existants quand la période d'emploi bouge ; `decide_leave` ne revérifie pas la période d'emploi ; rien n'empêche un `update()` brut de déplacer les dates d'un congé décidé (seul chemin réaliste : un import de masse) ; aucun trigger PostgreSQL sur les 7 tables (même dette que S3/S5/S6) ; `LeaveRequest.__str__` de l'admin Django porte le type à côté d'un nom (superuser seul, aucun droit d'API).
...
- **S8 (guardian, 15/08/2026)** : `update_equipment` sur une fiche qu'un autre directeur vient de réformer échoue avec le **message de terminalité** alors qu'il ne touchait pas au statut — refus fail-closed, message trompeur · pas de trigger PostgreSQL sur les 2 tables (un `UPDATE` brut ressusciterait un appareil réformé ; aucune route ne l'expose) · `GET /centers/{c}/equipment/` non paginé (choix assumé, même posture que les chambres et l'annuaire RH).
...
- **S6 (guardian)** : **export RGPD sans clé `stays`** — `discharged_at`, `priority` et l'historique des lits n'apparaissent nulle part dans l'export art. 20 ; la justification « l'épisode reste lisible par sa consultation pivot » est incomplète. Ajout estimé à six lignes (`StayPatientSerializer` est déjà la fenêtre du patient).
- **S6 (guardian)** : aucun trigger PostgreSQL sur les 4 tables du module (append-only de `BedAssignment` et finalité de l'état terminal de `Stay` vivent dans `save()`) — **à solder EN BLOC avec les tables de S3 et S5**.
- **S6 (guardian)** : la fusion de doublons de deux hospitalisés laisse **deux séjours en cours** sur le profil canonique (prouvé). Ne PAS bloquer la fusion est le bon arbitrage — l'anomalie est visible au tableau d'occupation, et la sortir automatiquement serait une décision clinique que le code n'a pas à prendre.
- **S6 (guardian)** : admission antidatée jusqu'à 366 jours → l'historique d'un lit peut porter deux occupations qui se chevauchent dans le temps (l'exclusivité *courante* n'est pas atteinte) ; **aucune route ne désactive une chambre ou un lit** (`is_active` honoré par les querysets mais inatteignable, l'admin étant en lecture seule) ; `set_attending` (service) n'exige pas un rôle clinique ; `priority` lisible par tout le staff administratif (conforme à la décision 5, mais « critique » est un signal clinique doux).
...
- **S7 (guardian backend)** : aucun trigger PostgreSQL sur les 7 tables RH (même dette qu'en S3/S6) — archivage définitif du justificatif et états terminaux vivent dans `save()` + 2 `CheckConstraint`.
