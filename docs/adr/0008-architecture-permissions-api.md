...
## Addendum S2 — Casquettes & parcours complétés (2026-08-13)

Quatre trous de parcours de l'audit C.4 fermés, chacun documenté à sa place ; ici le volet permissions :

1. **RDV côté patient** (`GET /patients/me/appointments/`, `POST .../{pk}/cancel/`) — `IsPatientSelf`, queryset `for_patient()`, payload dédié sans `reason` : addendum S2 de l'ADR 0013.
2. **Consentement clinique au guichet** (`POST|DELETE /centers/{c}/patients/{pk}/consents/clinical/`) — BILLING, patients NON revendiqués seuls : addendum S2 de l'ADR 0004.
3. **`GET /guardian/links/` — historique des liens, tous statuts** : porte `IsGuardian` (norme S1 — l'endpoint liste les liens EUX-MÊMES, objets de cycle de vie, pas des données scopées ; et le tuteur dont le dernier lien vient d'être suspendu par la porte de confirmation ne détient AUCUN scope actif — c'est exactement lui qui a besoin de cette liste : avant S2 son protégé « disparaissait » de `/proteges/` sans explication pendant `attente_confirmation_titulaire`, et les liens révoqués n'avaient aucun historique). Payload minimum administratif strict, plus étroit que `/proteges/` : `{id, protege_display_name, status, created_at, revoked_at}` — ni téléphone, ni `claim_status`, ni `relationship`, ni scopes : une ligne non active ne doit jamais devenir une fenêtre latérale sur le protégé. Les contrats `proteges`/`invitations` sont inchangés (lecture additive).
4. **`PATCH /guardian/profile/`** — champ par champ : `country_of_residence` modifiable (on déménage) ; `preferred_currency` modifiable car NON consommée par les devis (les devis sont structurellement EUR→KMF, taux et montants figés sur l'intent — ADR 0009 ; si un devis multi-devise existe un jour, il figera SA devise, jamais une lecture vive de ce champ) ; identité (prénom/nom) sur `User` → déjà couverte par `PATCH /auth/me/`. La vue `/guardian/profile/` reste `IsAuthenticated` + 404 sans profil (exemption documentée du test structurel : c'est le « guichet d'entrée » de l'espace — même information qu'un 403 `IsGuardian`, contrat GET conservé). Non audité (même classe que la création de profil : ni argent, ni médical, ni consentement, ni rôle).
